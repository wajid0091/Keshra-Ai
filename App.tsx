...
import { 
  ...
  ThumbsUp, ThumbsDown, LogOut, Lock, Mail, Heart, CreditCard, Coins, Brain, Square, Share2
} from 'lucide-react';
                        ...
                        {m.type === 'image' && (
                          <div className="space-y-3">
                             ...
                             <div className="flex gap-2">
                                 <a href={m.content} download={`keshra-${m.id}.png`} className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border ${borderCol} hover:bg-slate-50 dark:hover:bg-white/5 transition-all text-sm font-bold ${textColor}`}>
                                    <Download className="w-4 h-4" /> Download
                                 </a>
                                 <button onClick={() => {
                                     if (navigator.share) {
                                         fetch(m.content).then(res => res.blob()).then(blob => {
                                            const file = new File([blob], `keshra-${m.id}.png`, { type: 'image/png' });
                                            navigator.share({
                                                title: 'Keshra AI Image',
                                                text: 'Check out this image generated by Keshra AI!',
                                                files: [file]
                                            }).catch(console.error);
                                         });
                                     } else {
                                 ...
                                 }} className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border ${borderCol} hover:bg-slate-50 dark:hover:bg-white/5 transition-all text-sm font-bold ${textColor}`}>
